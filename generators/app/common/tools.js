...
export function copyTplFileList(generator, scanPath, destinationPath, extensionConfig, exclude = []) {
    ...
    loadFileList(generator.templatePath(scanPath), exclude)
        ...
        generator.fs.copyTpl(generator.templatePath(sourcePath + item), generator.destinationPath(destPath + item), extensionConfig);
    });
    // generator.log('==============================')
}
export function copyTplDir(generator, scanPath, destinationPath, extensionConfig, exclude = []) {
    const destPath = destinationPath.length > 0 ? destinationPath + '/' : '';
    const sourcePath = scanPath.length > 0 ? scanPath + '/' : '';
...
export function copyTplLibrary(generator, scanPath, destinationPath, sourcePackageDir, targetPackageDir, extensionConfig, exclude = [], replace = []) {
    ...
    getAllFile(generator.templatePath(scanPath)).filter((item) => {
        ...
        generator.fs.copyTpl(generator.templatePath(sourcePath + item), generator.destinationPath(destPath + savePath), extensionConfig);
    });
    // generator.log('==============================')
}
